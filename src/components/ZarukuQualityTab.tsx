...
export default function ZarukuQualityTab({ data }: { data: ZarukuSeoData }) {
  ...
  const limitedDatasets = datasets.filter(([, meta]) => meta.state === "partial" || meta.state === "unavailable");
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-xl border border-slate-200 bg-white"><header className="border-b border-slate-100 px-5 py-4"><h3 className="text-base font-semibold text-slate-900">Покрытие и ограничения</h3><p className="mt-1 text-xs text-slate-500">Что можно интерпретировать сейчас и где выводы ограничены источником или уровнем детализации.</p></header><div className="space-y-5 px-5 py-4"><div className="grid gap-3 md:grid-cols-2">{data.data_quality.map((item) => <div key={item.title} className="rounded-lg bg-slate-50 px-4 py-3"><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-semibold text-slate-700">{item.title}</div><div className="mt-1 text-xs leading-relaxed text-slate-500">{item.note}</div></div><div className={item.severity === "warning" ? "text-sm font-medium text-amber-700" : "text-sm font-medium text-slate-600"}>{item.value}</div></div></div>)}</div>{limitedDatasets.length ? <div><h4 className="text-sm font-semibold text-slate-800">Ограниченные наборы</h4><div className="mt-2 divide-y divide-slate-100 rounded-lg border border-slate-200">{limitedDatasets.map(([key, meta]) => <div key={key} className="flex flex-col gap-1 px-3 py-2.5 sm:flex-row sm:items-start sm:justify-between"><span className="text-sm font-medium text-slate-700">{DATASET_LABELS[key]}</span><span className="max-w-2xl text-xs leading-relaxed text-slate-500">{meta.state === "unavailable" ? "Источник данных временно недоступен." : "Данные доступны не за весь выбранный период."}</span></div>)}</div></div> : null}</div></section>

      <section className="rounded-xl border border-slate-200 bg-white"><header className="border-b border-slate-100 px-5 py-4"><h3 className="text-base font-semibold text-slate-900">Свежесть источников</h3><p className="mt-1 text-xs text-slate-500">Здесь видно, когда данные обновлялись и были ли задержки. Служебные сведения доступны в деталях.</p></header><div className="px-5 py-4">{data.source_freshness.length ? <div className="space-y-3">{data.source_freshness.map((row) => <div key={row.source_key} className="rounded-lg border border-slate-200 px-4 py-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-sm font-semibold text-slate-800">{row.label}</div><div className="mt-1 text-xs text-slate-500">Данные: {row.date_from && row.date_to ? `${row.date_from} — ${row.date_to}` : "период не подтверждён"} · последнее успешное обновление {row.last_success_at ?? "—"}</div></div><span className={`rounded-md px-2 py-1 text-xs font-semibold ${statusClass(row.freshness_status)}`}>{statusLabel(row.freshness_status)}</span></div><details className="mt-3 text-xs text-slate-500"><summary className="cursor-pointer font-medium text-slate-600">Технические детали</summary><div className="mt-2 grid gap-1 rounded-md bg-slate-50 p-3 sm:grid-cols-2"><div>Процесс обновления: {row.collector}</div><div>Ожидаемый ритм: {row.expected_frequency_hours} ч.</div><div>Прочитано строк: {row.rows_read.toLocaleString("ru-RU")}</div><div>Записано строк: {row.rows_written.toLocaleString("ru-RU")}</div><div className="sm:col-span-2">{row.note}</div>{row.last_error_summary ? <div className="text-red-700 sm:col-span-2">{row.last_error_at ?? "Ошибка"}: {row.last_error_summary}</div> : null}</div></details></div>)}</div> : <div className="rounded-lg bg-slate-50 px-4 py-6 text-sm text-slate-500">История обновлений пока не накоплена.</div>}</div></section>
