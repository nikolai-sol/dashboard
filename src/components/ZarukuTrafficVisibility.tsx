...
export default function ZarukuTrafficVisibility({ seoOs, primaryWeek, comparisonWeek, source }: Props) {
  ...
  const chartRows = rows.map((row) => ({
    section: row.section,
    primary_visits: row.primary.visits,
    comparison_visits: row.comparison?.visits ?? null,
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white">
      ...
      <div className="border-b border-slate-100 px-5 py-4">
        {chartRows.length ? <ResponsiveContainer width="100%" height={300}><ComposedChart data={chartRows} margin={{ top: 10, right: 12, left: -12, bottom: 0 }}><CartesianGrid stroke="#eef2f7" strokeDasharray="3 3" vertical={false} /><XAxis dataKey="section" tick={{ fontSize: 12, fill: "#64748b" }} axisLine={false} tickLine={false} /><YAxis yAxisId="traffic" tick={{ fontSize: 12, fill: "#64748b" }} axisLine={false} tickLine={false} /><YAxis yAxisId="position" orientation="right" domain={[1, "dataMax + 1"]} reversed allowDecimals={false} tick={{ fontSize: 12, fill: "#64748b" }} axisLine={false} tickLine={false} label={{ value: "Позиция", angle: 90, position: "insideRight", fill: "#64748b", fontSize: 12 }} /><Tooltip /><Legend wrapperStyle={{ fontSize: 12 }} /><Bar yAxisId="traffic" dataKey="primary_visits" name={`A ${primaryWeek ?? ""} visits`} fill="#0d9488" radius={[4, 4, 0, 0]} />{comparisonWeek ? <Bar yAxisId="traffic" dataKey="comparison_visits" name={`B ${comparisonWeek} visits`} fill="#94a3b8" radius={[4, 4, 0, 0]} /> : null}<Line yAxisId="position" type="linear" dataKey="primary_position" name={`A ${primaryWeek ?? ""} position`} stroke="#9333ea" strokeWidth={2.5} dot={{ r: 3 }} connectNulls={false} />{comparisonWeek ? <Line yAxisId="position" type="linear" dataKey="comparison_position" name={`B ${comparisonWeek} position`} stroke="#64748b" strokeWidth={2} strokeDasharray="6 4" dot={{ r: 2 }} connectNulls={false} /> : null}</ComposedChart></ResponsiveContainer> : <div className="flex h-[300px] items-center justify-center text-sm text-slate-500">Нет трафика или видимости для выбранной недели.</div>}
      </div>
      <div className="max-h-[360px] overflow-auto px-5 py-4">
        <table className="w-full min-w-[920px] text-sm"><thead><tr className="text-left text-xs uppercase text-slate-400"><th className="pb-2 font-medium">Раздел</th><th className="pb-2 text-right font-medium">A visits</th>{comparisonWeek ? <><th className="pb-2 text-right font-medium">B visits</th><th className="pb-2 text-right font-medium">Delta</th></> : null}<th className="pb-2 text-right font-medium">A позиция</th>{comparisonWeek ? <><th className="pb-2 text-right font-medium">B позиция</th><th className="pb-2 text-right font-medium">Delta</th></> : null}<th className="pb-2 text-right font-medium">Покрытие A</th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map((row) => <tr key={row.section}><td className="py-2.5 font-medium text-slate-700">{row.section}</td><td className="py-2.5 text-right text-slate-600">{formatNumber(row.primary.visits)}</td>{comparisonWeek ? <><td className="py-2.5 text-right text-slate-600">{formatNumber(row.comparison?.visits ?? null)}</td><td className={row.visits_delta != null && row.visits_delta < 0 ? "py-2.5 text-right text-red-700" : "py-2.5 text-right text-teal-700"}>{row.visits_delta == null ? "—" : `${row.visits_delta > 0 ? "+" : ""}${formatNumber(row.visits_delta)}`}</td></> : null}<td className="py-2.5 text-right text-slate-600">{formatNumber(row.primary.average_position)}</td>{comparisonWeek ? <><td className="py-2.5 text-right text-slate-600">{formatNumber(row.comparison?.average_position ?? null)}</td><td className={row.position_delta != null && row.position_delta > 0 ? "py-2.5 text-right text-red-700" : "py-2.5 text-right text-teal-700"}>{positionDelta(row.position_delta)}</td></> : null}<td className="py-2.5 text-right text-slate-600">{row.primary.coverage == null ? "—" : `${formatNumber(row.primary.coverage * 100)}%`}</td></tr>)}{rows.length === 0 ? <tr><td colSpan={comparisonWeek ? 8 : 4} className="py-6 text-center text-sm text-slate-500">Нет строк из словаря разделов.</td></tr> : null}</tbody></table>
      </div>
    </section>
  );
}
