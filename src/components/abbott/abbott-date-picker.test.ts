...
test("Abbott picker announces custom validation while the page owns the empty-period status", () => {
  ...
  assert.doesNotMatch(pickerSource, /emptyMessage/);
  assert.match(pageSource, /role="status" aria-live="polite"/);
  assert.doesNotMatch(pageSource, /emptyMessage=\{abbottEmptyMessage\}/);
});
