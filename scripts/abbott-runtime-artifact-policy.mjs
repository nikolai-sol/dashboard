...
const ABBOTT_APP_ROOT = "apps/abbott";
...
const ABBOTT_TRACE_INCLUDES = { "/*": ["./src/schemas/yandex_metrika.yaml"] };
const ABBOTT_TRACE_EXCLUDES = { "/*": ["./src/**/*.test.ts", "./src/**/*.test.tsx", "./src/**/*fixture*.ts"] };
const REPOSITORY_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const ABBOTT_STAMP_HELPER = path.join(REPOSITORY_ROOT, "scripts/abbott-artifact-stamp.py");
...
const ABBOTT_COMPILED_ROUTES = new Set(ABBOTT_ROUTES.values());
const ABBOTT_DYNAMIC_ROUTES = [
...
const ASSET_REWRITE = {
  source: "/_next-abbott/_next/:path+", destination: "/_next/:path+",
  regex: "^/_next-abbott/_next(?:/((?:[^/]+?)(?:/(?:[^/]+?))*))(?:/)?$",
};
...
function inspectAbbottRoutes(files, violations) {
  const json = (name) => {
    const filename = `${ABBOTT_NEXT}/${name}`;
  ...
  const check = (name, predicate) => {
    ...
    try { if (value === null || !predicate(value)) violations.push(`invalid route authority ${ABBOTT_NEXT}/${name}`); }
    catch { violations.push(`invalid route authority ${ABBOTT_NEXT}/${name}`); }
    return value;
  };
  const manifest = json("server/app-paths-manifest.json") ?? {};
  for (const [route, target] of ABBOTT_ROUTES) {
    if (manifest[route] !== target) {
      violations.push(`missing route ${route} -> ${target}`);
      continue;
    }
    if (!files.has(`${ABBOTT_NEXT_SERVER}/${target}`)) violations.push(`missing compiled route ${target}`);
  }
  for (const route of Object.keys(manifest)) {
    if (!ABBOTT_ROUTES.has(route)) violations.push(`unexpected route ${route}`);
  }
  ...
  check("server/functions-config-manifest.json", (v) => isDeepStrictEqual(v, { version: 1, functions: { "/api/health": {} } }));
  ...
  const routePaths = [...ABBOTT_ROUTES.keys()].map((route) => route.replace(/\/(page|route)$/, ""));
  check("app-path-routes-manifest.json", (v) => isDeepStrictEqual(v, Object.fromEntries([...ABBOTT_ROUTES.keys()].map((route, i) => [route, routePaths[i]]))));
  check("routes-manifest.json", (v) => v.version === 3 && v.appType === "app" && v.basePath === "" &&
    ...
    isDeepStrictEqual([...v.dynamicRoutes].sort((a, b) => a.page.localeCompare(b.page)), [...ABBOTT_DYNAMIC_ROUTES].sort((a, b) => a.page.localeCompare(b.page))) &&
    isDeepStrictEqual(v.dataRoutes, []) &&
    isDeepStrictEqual([...v.staticRoutes].sort((a, b) => a.page.localeCompare(b.page)), routePaths.filter((page) => !page.includes("[id]")).sort().map((page) => {
  ...
  const required = check("required-server-files.json", (v) => v.version === 1 && v.relativeAppDir === ABBOTT_APP_ROOT &&
    v.config.output === "standalone" && v.config.distDir === ".next-abbott" && v.config.assetPrefix === "/_next-abbott" &&
    v.config.basePath === "" && !v.config.i18n && isDeepStrictEqual(v.config.env, {}) &&
    isDeepStrictEqual(v.config.outputFileTracingIncludes, ABBOTT_TRACE_INCLUDES) &&
    isDeepStrictEqual(v.config.outputFileTracingExcludes, ABBOTT_TRACE_EXCLUDES) &&
    ...
    Array.isArray(v.files) && v.files.length > 0 && v.files.every((file) => typeof file === "string" &&
      file.startsWith(".next-abbott/") && !file.split("/").includes("..") && files.has(`${ABBOTT_APP_ROOT}/${file}`)));
  if (!files.get(`${ABBOTT_NEXT}/BUILD_ID`)?.text?.match(/^[\w-]+$/)) violations.push(`invalid ${ABBOTT_NEXT}/BUILD_ID`);
  try {
    const source = files.get(ABBOTT_SERVER).text;
    ...
    if (normalized(source.replace(match[0], "const nextConfig = {}")) !== normalized(STANDALONE_TEMPLATE) ||
      !isDeepStrictEqual({ ...config, distDir: ".next-abbott" }, required.config) || config.distDir !== "./.next-abbott") throw new Error();
  } catch { violations.push(`invalid standalone server ${ABBOTT_SERVER}`); }
}
...
function inspectFileClosure(files, violations, { rejectUntraced = true } = {}) {
  ...
  for (const code of [404, 500]) {
    const filename = `${ABBOTT_NEXT_SERVER}/pages/${code}.html`;
    if (files.has(filename)) include(filename);
  }
  ...
  const buildId = files.get(`${ABBOTT_NEXT}/BUILD_ID`)?.text;
  const staticName = (name) => /^static\/(?:chunks\/(?:(?:[\w-]+|%5Bid%5D)\/)*[\w.-]+\.js|css\/[\w.-]+\.css|media\/[\w.-]+\.(?:woff2?|ttf|otf|png|jpe?g|gif|webp|avif|ico|svg))$/.test(name) ||
  ...
  for (const name of ["build-manifest.json", "react-loadable-manifest.json", "server/next-font-manifest.json"]) {
    try { addAssets(JSON.parse(files.get(`${ABBOTT_NEXT}/${name}`).text)); }
    catch { violations.push(`invalid asset authority ${ABBOTT_NEXT}/${name}`); }
  }
  for (const [route, target] of ABBOTT_ROUTES) {
    const name = `${ABBOTT_NEXT_SERVER}/${target.replace(/\.js$/, "_client-reference-manifest.js")}`;
  ...
  for (const name of allowed) {
    ...
    try {
      ...
      let expected;
      if (name === `${ABBOTT_NEXT}/package.json`) expected = { type: "commonjs" };
      else {
...
export function prepareRuntimeArtifactFiles(artifactRoot, traceRewrites) {
  ...
  if (traceRewrites.size !== 0) throw new Error("build emitted forbidden test trace material");
  const schemaSource = readStableFile(path.join(REPOSITORY_ROOT, ABBOTT_SCHEMA_SOURCE)).buffer;
  let schemaArtifact;
  try { schemaArtifact = readStableFile(path.join(root, ABBOTT_SCHEMA_ARTIFACT)).buffer; }
  catch { throw new Error("build omitted required Abbott schema"); }
  if (!schemaArtifact.equals(schemaSource)) throw new Error("built Abbott schema differs from source authority");
  if (fs.existsSync(path.join(root, ABBOTT_SCHEMA_SOURCE))) throw new Error("build emitted obsolete root schema");
  const sourceRoot = path.join(root, ABBOTT_APP_ROOT, "src");
  const visit = (directory, depth = 0) => {
    if (depth > 64) throw new Error("excessive built source depth");
    for (const name of fs.readdirSync(directory).sort()) {
      const absolute = path.join(directory, name);
      const stat = fs.lstatSync(absolute);
      const relative = normalizedPath(root, absolute);
      if (stat.isSymbolicLink()) throw new Error(`built source symlink forbidden ${relative}`);
      if (stat.isDirectory()) visit(absolute, depth + 1);
      else if (isAbbottTestMaterial(relative)) throw new Error(`build emitted test material ${relative}`);
    }
  };
  visit(sourceRoot);
}
...
function loadTrustedManifest(root, filename) {
  ...
  if (value.version !== 1 || value.scope !== "abbott" || !SOURCE_SHA_PATTERN.test(value.sourceSha) ||
    ...
    !isDeepStrictEqual(value.dynamic, []) ||
  ...
  for (const entry of value.files) {
    if (!entry || typeof entry.path !== "string" || !entry.path ||
      ...
      !entry.required && !entry.path.startsWith(`${ABBOTT_NEXT}/static/`)) throw new Error("invalid trusted file entry");
...
export function createTrustedRuntimeManifest(artifactRoot, scope, sourceSha, manifestPath) {
  ...
  const config = JSON.parse(files.get(`${ABBOTT_NEXT}/required-server-files.json`).text).config;
  ...
  const server = STANDALONE_TEMPLATE.split("\n").map((line) => starts.some((start) => line.startsWith(start)) ? `\n${line}` : line).join("\n")
    .replace("const nextConfig = {}", `const nextConfig = ${JSON.stringify({ ...config, distDir: "./.next-abbott" })}`);
  put(ABBOTT_SERVER, Buffer.from(server));
  put(".release-source-sha", Buffer.from(`${sourceSha}\n`));
  put(".release-runtime-scope", Buffer.from(`${scope}\n`));
  for (const target of ABBOTT_COMPILED_ROUTES) { load(`${ABBOTT_NEXT_SERVER}/${target}`); load(`${ABBOTT_NEXT_SERVER}/${target}.nft.json`); }
  for (const route of ["_global-error", "_not-found"]) {
    const stem = `${ABBOTT_NEXT_SERVER}/app/${route}`;
  ...
  for (const code of [404, 500]) load(`${ABBOTT_NEXT_SERVER}/pages/${code}.html`, true);
  ...
  const manifest = { version: 1, scope, sourceSha, next: { name: "next", version: "16.1.6" },
    ...
    files: [...allowed].sort().map((name) => { const file = files.get(name); return { path: name, type: "file", mode: file.mode, size: file.size,
      sha256: file.digest, required: !name.startsWith(`${ABBOTT_NEXT}/static/`) }; }),
  };
...
function inspectPath(relativePath, violations) {
  ...
  if (relativePath.startsWith(`${ABBOTT_APP_ROOT}/`) && ![
    ABBOTT_SERVER, `${ABBOTT_APP_ROOT}/package.json`, ABBOTT_NEXT,
  ].includes(relativePath) && !relativePath.startsWith(`${ABBOTT_NEXT}/`) &&
    !relativePath.startsWith(`${ABBOTT_APP_ROOT}/src/`) && relativePath !== `${ABBOTT_APP_ROOT}/src`) {
    violations.push(`unexpected app file ${relativePath}`);
  }
  if (relativePath.startsWith(`${ABBOTT_NEXT_SERVER}/`) && /\.(?:[cm]?js)$/.test(relativePath)) {
    const local = relativePath.slice(`${ABBOTT_NEXT_SERVER}/`.length);
  ...
  if ((/\/(?:\.next[^/]*|server)\//.test(relativePath) && !relativePath.startsWith(`${ABBOTT_NEXT}/`) && !relativePath.startsWith("node_modules/")) ||
    (lower.startsWith(`${ABBOTT_NEXT_SERVER}/edge`) || lower.startsWith(`${ABBOTT_NEXT_SERVER}/pages/`) && !["404.html", "500.html"].includes(basename))) {
    violations.push(`foreign executable tree ${relativePath}`);
  }
  if (lower.startsWith(`${ABBOTT_NEXT_SERVER}/app/`) && /\/(?:page|route)\.js$/.test(lower)) {
    const route = relativePath.slice(`${ABBOTT_NEXT_SERVER}/`.length);
    if (!ABBOTT_COMPILED_ROUTES.has(route)) violations.push(`unexpected compiled route ${route}`);
  }
}
...
function inspectRuntimeArtifactState(artifactRoot, scope, trustedManifestPath) {
  ...
  if (scope !== "abbott") return rejected(`unsupported runtime scope ${scope}`);
...
export async function verifyRuntimeArtifactBoot(artifactRoot, scope, { timeoutMs = 15000, trustedManifestPath } = {}) {
  ...
  try {
    ...
    child = spawn(process.execPath, [path.join(root, ABBOTT_SERVER)], {
      cwd: root,
    ...
    while (Date.now() < deadline && child.exitCode === null && !childError) {
      try {
        ...
        if (response.status !== 503 || response.headers.get("cache-control") !== "private, no-store" || body.length > 1024 ||
          !isDeepStrictEqual(JSON.parse(body), { ok: false, scope: "abbott", database: "disconnected" })) {
  ...
  } catch { throw new Error("Abbott standalone boot/health check failed"); }
  finally {
    if (child) {
      child.kill("SIGTERM");
...
function stampOperation(action, relativePath, data, mode, expected) {
  return {
    action,
    path: relativePath,
    ...
    mode,
    ...(expected ? { expectedSha256: createHash("sha256").update(expected).digest("hex") } : {}),
  };
}
...
function stampRuntimeFiles(root, sourceTrace, sourceSha, trusted) {
  ...
  const runtimePackage = Buffer.from(`${JSON.stringify(runtimePackageJson, null, 2)}\n`);
  const scope = Buffer.from("abbott\n");
  const sha = Buffer.from(`${sourceSha}\n`);
  const request = {
    version: 1,
    root,
    operations: [
      ...
      stampOperation("create", ".release-runtime-scope", scope, 0o644),
    ],
  };
  const helper = readStableFile(ABBOTT_STAMP_HELPER);
  if (!helper.buffer.length) throw new Error("invalid Abbott stamp helper");
  execFileSync("python3", [ABBOTT_STAMP_HELPER], {
...
export function stampRuntimeArtifact(artifactRoot, scope, sourceSha, trustedManifestPath) {
  if (scope !== "abbott") throw new Error(`unsupported runtime scope ${scope}`);
  ...
  if (!isRegularFile(path.join(root, ABBOTT_SERVER))) throw new Error(`missing ${ABBOTT_SERVER}`);
...
function usage() {
  return "usage: abbott-runtime-artifact-policy.mjs [--prepare|--stamp|--verify|--boot] abbott <artifact-root> --trusted-manifest <external-path>";
}
...
async function main() {
  ...
  const positional = prepare || stamp || verify || boot ? args.slice(1) : args;
  ...
  if (prepare || stamp) {
    ...
    console.log("Abbott standalone loopback boot/health passed");
  } else {
    assertRuntimeArtifact(artifactRoot, scope, trustedManifestPath);
    if (verify) console.log("Abbott trusted runtime artifact passed");
  }
}
