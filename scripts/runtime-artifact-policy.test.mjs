...
test("review: workspace provides an explicit post-seal loopback boot gate", () => {
  ...
  assert.equal(pkg.scripts["verify:boot"], "node ../../scripts/runtime-artifact-policy.mjs --boot abbott .next-abbott/standalone --trusted-manifest .next-abbott/trusted-runtime-manifest.json");
});
...
test("the Abbott workspace stamps then verifies its standalone artifact", () => {
  ...
  assert.equal(
    ...
    "next build --webpack && node ../../scripts/runtime-artifact-policy.mjs --prepare abbott .next-abbott/standalone --trusted-manifest .next-abbott/trusted-runtime-manifest.json && node ../../scripts/runtime-artifact-policy.mjs --stamp abbott .next-abbott/standalone --trusted-manifest .next-abbott/trusted-runtime-manifest.json",
  );
  assert.equal(
    packageJson.scripts["verify:artifact"],
    "node ../../scripts/assert-abbott-artifact.mjs .next-abbott/standalone && node ../../scripts/runtime-artifact-policy.mjs --verify abbott .next-abbott/standalone --trusted-manifest .next-abbott/trusted-runtime-manifest.json",
  );
});
