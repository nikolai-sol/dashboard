...
test('lint ignores only generated runtime outputs and scratch while checking Abbott source', async () => {
  ...
  for (const filename of ['apps/abbott/.next-abbott/server/generated.js', '.superpowers/sdd/.abbott-deploy-evidence/scratch.cjs']) {
    assert.equal(await eslint.isPathIgnored(filename), true, `generated/scratch path must be ignored: ${filename}`);
  }
  for (const filename of ['apps/abbott/src/lint-probe.ts', 'apps/abbott/src/lint-probe.test.ts', 'scripts/abbott-deploy-runtime.mjs', 'deploy/abbott/start.cjs']) assert.equal(await eslint.isPathIgnored(filename), false);
