...
export function parseNginxNamesFrame(bytes){const invalid=refused('framing');if(!Buffer.isBuffer(bytes)||bytes.length>4096)return invalid;const text=bytes.toString(),r=/^ABBOTT_NGINX_READ_REFUSED reason=([a-z0-9_]+)\n$/.exec(text);if(r)return NGINX_READ_REASONS.includes(r[1])?text:invalid;const m=/^ABBOTT_NGINX_UNSUPPORTED names=([a-z0-9_,]+)\n$/.exec(text);if(!m)return invalid;if(m[1]==='none')return text;const names=m[1].split(','),known=new Set(abbottNginxDiagnosticNames());return names.every(n=>known.has(n))&&[...new Set(names)].sort().join(',')===m[1]?text:invalid;}
