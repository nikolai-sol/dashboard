...
import{abbottNginxDiagnosticNames}from'./runtime-release-remote.mjs';
const UNKNOWN='ABBOTT_NGINX_UNSUPPORTED names=other\n';
export function parseNginxNamesFrame(bytes){if(!Buffer.isBuffer(bytes)||bytes.length>4096)return UNKNOWN;const text=bytes.toString(),m=/^ABBOTT_NGINX_UNSUPPORTED names=([a-z0-9_,]+)\n$/.exec(text);if(!m)return UNKNOWN;if(m[1]==='none')return text;const names=m[1].split(','),known=new Set(abbottNginxDiagnosticNames());return names.every(n=>known.has(n))&&[...new Set(names)].sort().join(',')===m[1]?text:UNKNOWN;}
...
export const NGINX_SOURCE_HASHES=Object.freeze({"analysis":"351d868f6e629dcbbe9d00327eaeea5f646a751f1c7fc670c1cecdfc80f3429f","reader":"1897c17f6ba41c97dc8fa982b8760644a971b282b83ce10f1aa52909cb531bd0"});
const files={analysis:'runtime-release-remote.mjs',reader:'abbott-nginx-readonly.mjs'};
export const NGINX_READ_LOADER="import{createHash}from'node:crypto';let bytes=Buffer.alloc(0),done=false;const unknown=\"ABBOTT_NGINX_UNSUPPORTED names=other\\n\",pins={\"analysis\":\"351d868f6e629dcbbe9d00327eaeea5f646a751f1c7fc670c1cecdfc80f3429f\",\"reader\":\"1897c17f6ba41c97dc8fa982b8760644a971b282b83ce10f1aa52909cb531bd0\"};const finish=line=>{if(done)return;done=true;clearTimeout(timer);bytes.fill(0);process.stdout.write(line,()=>process.stdin.destroy());};const timer=setTimeout(()=>finish(unknown),25000);for(const s of ['SIGINT','SIGTERM','SIGHUP'])process.on(s,()=>finish(unknown));process.stdin.on('error',()=>finish(unknown));process.stdin.on('data',chunk=>{if(done){chunk.fill(0);return;}if(bytes.length+chunk.length>262144){chunk.fill(0);finish(unknown);return;}const next=Buffer.concat([bytes,chunk]);bytes.fill(0);chunk.fill(0);bytes=next;});process.stdin.on('end',async()=>{const buffers=[];try{if(done||process.getuid()!==0||process.argv.length!==1||Object.keys(process.env).some(k=>k!=='UV_USE_IO_URING'||process.env[k]!=='0'))throw Error();const input=JSON.parse(bytes);if(!input||JSON.stringify(input)!==bytes.toString()||Object.keys(input).sort().join(',')!=='analysis,reader')throw Error();const sources={};for(const key of ['analysis','reader']){if(typeof input[key]!=='string'||! /^[A-Za-z0-9+/]+={0,2}$/.test(input[key]))throw Error();const b=Buffer.from(input[key],'base64');buffers.push(b);if(b.toString('base64')!==input[key]||!b.length||b.length>131072||createHash('sha256').update(b).digest('hex')!==pins[key])throw Error();sources[key]=b;}const url=b=>'data:text/javascript;base64,'+b.toString('base64');let reader=sources.reader.toString();const needle=\"'./runtime-release-remote.mjs'\";if(reader.split(needle).length!==2)throw Error();reader=reader.replace(needle,JSON.stringify(url(sources.analysis)));const m=await import(url(Buffer.from(reader))),a=await import(url(sources.analysis));if(done)return;const names=m.readAbbottNginxNames(),known=new Set(a.abbottNginxDiagnosticNames());if(!Array.isArray(names)||names.some(n=>typeof n!=='string'||!known.has(n))||[...new Set(names)].sort().join(',')!==names.join(','))throw Error();const line='ABBOTT_NGINX_UNSUPPORTED names='+(names.length?names.join(','):'none')+'\\n';if(Buffer.byteLength(line)>4096)throw Error();finish(line);}catch{finish(unknown);}finally{for(const b of buffers)b.fill(0);bytes.fill(0);}});";
...
export function runNginxReadTransport(input,{platform=real,signal,onEvidence=()=>{}}={}){
 ...
 return new Promise(resolve=>{let child,pid=null,start=null,closed=false,exit=false,settled=false,ended=false,invalid=false,termSent=false,output=Buffer.alloc(0);const timers=[];
  ...
  const finish=()=>{if(closed)for(const timer of timers)platform.clearTimeout(timer);if(settled||!closed&&!ended)return;settled=true;
   ...
   const line=!invalid&&exitVerified?parseNginxNamesFrame(output):UNKNOWN;output.fill(0);resolve({line,exitVerified});};
...
export async function runNginxReadWithEvidence(input,{signal,transport=runNginxReadTransport,evidence}={}){
 let line=UNKNOWN;try{evidence??=createRecoveryEvidence();const result=await transport(input,{signal,onEvidence:r=>evidence.record(r)});const summary=evidence.finish();evidence=null;if(!signal?.aborted&&result.exitVerified&&summary.identityCaptured&&summary.exitObserved&&summary.exitVerified)line=parseNginxNamesFrame(Buffer.from(result.line));}catch{line=UNKNOWN;}finally{input.fill(0);if(evidence)try{evidence.finish();}catch{line=UNKNOWN;}}return line;
}
...
async function main(){let input;const sources=[],abort=new AbortController(),stop=()=>abort.abort();for(const name of['SIGINT','SIGTERM','SIGHUP'])process.on(name,stop);
 try{const git=verifyRecoveryLocalAuthority();git('merge-base','--is-ancestor','9fb03e3d17284c6fba6c6bbfb9edefc4797772d6','HEAD');if(fs.realpathSync(process.argv[1])!=='/Users/nafanya/ReportingDash/dashboard-next/.worktrees/abbott-runtime-isolation/scripts/read-abbott-nginx.mjs'||!git('show','HEAD:scripts/read-abbott-nginx.mjs').equals(fs.readFileSync(new URL(import.meta.url))))throw Error();
  ...
  input=buildNginxReadInput(map);const line=await runNginxReadWithEvidence(input,{signal:abort.signal});process.stdout.write(line);if(line===UNKNOWN)process.exitCode=1;
 }catch{process.stdout.write(UNKNOWN);process.exitCode=1;}finally{input?.fill(0);for(const b of sources)b.fill(0);for(const name of['SIGINT','SIGTERM','SIGHUP'])process.removeListener(name,stop);}
}
