...
function validRemoteDiagnostic(status,d){
 ...
 if(status==='REFUSED'&&d.stage==='persistence')return ['failed','neighbor_saved_drift'].includes(d.reason);
...
export function parseAbbottDeployResult(text,digest){
 ...
 const m=/^ABBOTT_DEPLOY_RESULT ([a-f0-9]{64}) ([^\n]+)\nABBOTT_DEPLOY_ACK (COMMITTED|RESTORED|REFUSED|REVIEW_REQUIRED) ([a-f0-9]{64}) ([a-f0-9]{32}|none) ([a-z_]+) ([a-z0-9_]+)\n$/.exec(text);if(!m||m[1]!==digest||m[4]!==digest)return null;
...
export const DEPLOY_STAGES=Object.freeze(['local_spawn','identity_proof','source_write','run_write','remote_startup','ack_framing','timeout','ssh_close','local_evidence','complete','unknown','preflight_current','preflight_browser','preflight_nginx','preflight_neighbor_combined','preflight_neighbor_zaruku','preflight_neighbor_medroche','lock','prepare','activation_precheck','activation_stop','activation_start','candidate_health','pointer','compensation','persistence']);
export const DEPLOY_REASONS=Object.freeze(['neighbor_saved_drift','failed','unavailable','unverified','malformed','missing','oversized','stderr','deadline','nonzero','signal','none','unknown','restored','review_required','pid_absent','start_mismatch','uid_gid','cwd','release_record','executable','cmdline','listener','proc_metadata','metadata','utf8','syntax','tls_count','include','nested_server','variable_routing','regex_location','unsupported_directive','existing_abbott_route','existing_3004','snapshot_drift','unsupported_location','unsupported_proxy_pass','unsupported_return','unsupported_add_header','unsupported_root','unsupported_alias','unsupported_index','unsupported_try_files','unsupported_error_page','unsupported_proxy_redirect','unsupported_proxy_cache','unsupported_ssl_ecdh_curve','unsupported_ssl_conf_command','unsupported_client_body_buffer_size','unsupported_charset','unsupported_gzip_vary','unsupported_if','unsupported_other']);
...
async function execute(){running=true;let entered=false;try{if(abort.signal.aborted)throw Error();const m=await import('data:text/javascript;base64,'+source.toString('base64'));if(abort.signal.aborted)throw Error();const request=JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(payload));if(!['inspect','deploy','rollback','cold-restore-current'].includes(request?.action))throw Error();entered=true;const result=await m.run(abort.signal,request);finish(result);}catch{finish(entered?null:{status:'REFUSED',record:null,diagnostic:{stage:'unknown',reason:'failed'}});}}
...
while(pending.length&&!finished){
 ...
 }else{if(running)stop();if(controlDone){invalid();return;}const allowed=running?['ABORT\n']:['RUN\n','ABORT\n'],end=pending.indexOf(10);if(end<0){if(!allowed.some(line=>line.startsWith(pending.toString('latin1'))))invalid();return;}const line=pending.subarray(0,end+1).toString('latin1'),rest=Buffer.from(pending.subarray(end+1));pending.fill(0);pending=rest;if(line==='ABORT\n'){controlDone=true;stop();if(!running){finish();return;}}else if(line==='RUN\n'&&!running&&!abort.signal.aborted){void execute();}else{invalid();return;}}
}}
process.stdin.on('data',consume);process.stdin.on('end',()=>{if(pending.length)rejectPending();stop();if(!running)finish();});process.stdin.on('error',()=>{if(pending.length)rejectPending();stop();if(!running)finish();});process.stdout.write('ABBOTT_DEPLOY_READY\n');`;
