...
test('fixed inventory is dependency-free authority and provisioning closure only', () => {
  assert.deepEqual(CONTROL_FILES, [
    ...
    'scripts/zaruku-production-shadow-worker.mjs', 'scripts/zaruku-shadow-coverage.mjs', 'scripts/zaruku-shadow-db.mjs', 'scripts/zaruku-shadow-evidence-lock.py', 'scripts/zaruku-shadow-host.mjs',
...
test('pure runtime authority loads without TS or any node_modules dependency', () => {
  ...
  for (const name of ['zaruku-shadow-host.mjs', 'zaruku-shadow-db.mjs']) assert.match(fs.readFileSync(path.join(import.meta.dirname, name), 'utf8'), /from '\.\/zaruku-production-shadow-authority\.mjs'/);
});
