...
const INCLUDES=['/etc/nginx/snippets/coopervision-market-path.conf','/etc/nginx/snippets/reportingdash-public-coopervision-market-intelligence-c.conf'];
function withIncludes(f){f.files.set(NGINX,NGINX_TEXT.replace('location / {',INCLUDES.map(p=>'include '+p+';').join('')+'location / {'));for(const [i,file]of INCLUDES.entries()){f.files.set(file,'location /fixture'+i+' { proxy_pass http://127.0.0.1:300'+(i+1)+'; }');f.metadata.set(file,{mode:0o100644});}return f;}
test('fixed include proof snapshots both files and latches fragment drift after reversion',async()=>{const m=await api(),f=withIncludes(fixture()),notes=[];f.options.notePhase=(stage,reason)=>notes.push({stage,reason});const proof=m.createAbbottDeploymentProof(f.options);assert.doesNotThrow(()=>proof.preflight());assert.ok(INCLUDES.every(p=>f.opened.includes(p)));proof.perimeter();const old=f.files.get(INCLUDES[0]);f.files.set(INCLUDES[0],old+' # changed');assert.throws(()=>proof.perimeter());assert.equal(notes.at(-1).reason,'include_snapshot_drift');f.files.set(INCLUDES[0],old);assert.throws(()=>proof.perimeter());assert.equal(f.fds.size,0);assert.ok(f.buffers.every(b=>b.every(v=>v===0)));});
test('include metadata and content failures refuse with no mutation and zero buffers',async()=>{const m=await api();for(const mode of ['missing','symlink','hardlink','owner','group','mode','ancestor','swap','oversize','utf8','syntax','route']){const f=withIncludes(fixture()),file=INCLUDES[0],notes=[];f.options.notePhase=(stage,reason)=>notes.push({stage,reason});if(mode==='missing')f.files.delete(file);if(mode==='symlink')f.links.set(file,'/private');if(mode==='hardlink')f.metadata.set(file,{mode:0o100644,nlink:2});if(mode==='owner')f.metadata.set(file,{mode:0o100644,uid:9});if(mode==='group')f.metadata.set(file,{mode:0o100644,gid:9});if(mode==='mode')f.metadata.set(file,{mode:0o100640});if(mode==='ancestor')f.metadata.set('/etc/nginx/snippets',{mode:0o40777});if(mode==='oversize')f.files.set(file,'x'.repeat(65537));if(mode==='syntax')f.files.set(file,'location /x {');if(mode==='route')f.files.set(file,'location /dashboard/18 { return 404; }');if(['swap','utf8'].includes(mode)){const read=f.options.io.readSync;f.options.io.readSync=(fd,b,...args)=>{const n=read(fd,b,...args);if(f.opened.at(-1)===file){if(mode==='swap')f.metadata.set(file,{mode:0o100644,ino:99});else if(n)b[0]=255;}return n;};}assert.throws(()=>m.createAbbottDeploymentProof(f.options).preflight());assert.equal(notes.at(-1).stage,'preflight_nginx');assert.ok(['include_metadata','include_utf8','include_syntax','include_route','include_existing_abbott_route'].includes(notes.at(-1).reason));assert.deepEqual(f.calls,[]);assert.equal(f.fds.size,0);assert.ok(f.buffers.every(b=>b.every(v=>v===0)));assert.doesNotMatch(JSON.stringify(notes),/private|\/etc|http/);}});
function fixture(){
...
test('no selected includes never reads snippet paths and invalid pair fails before any snippet read',async()=>{const m=await api();for(const body of [null,'include '+INCLUDES[0]+';','include /etc/nginx/unknown; include '+INCLUDES[1]+';']){const f=fixture();if(body!==null)f.files.set(NGINX,NGINX_TEXT.replace('location / {',body+'location / {'));const proof=m.createAbbottDeploymentProof(f.options);if(body===null){proof.preflight();proof.perimeter();}else assert.throws(()=>proof.preflight());assert.ok(!f.opened.some(p=>p.startsWith('/etc/nginx/snippets/')));assert.equal(f.fds.size,0);}});
test('second include and parent identity swaps refuse and latch across reversion',async()=>{const m=await api();for(const mode of ['second_missing','parent_symlink','parent_owner','parent_swap','second_metadata']){const f=withIncludes(fixture()),file=INCLUDES[1],notes=[];f.options.notePhase=(stage,reason)=>notes.push({stage,reason});const proof=m.createAbbottDeploymentProof(f.options);if(mode==='second_missing')f.files.delete(file);if(mode==='parent_symlink')f.links.set('/etc/nginx/snippets','/private');if(mode==='parent_owner')f.metadata.set('/etc/nginx/snippets',{uid:1});if(mode==='parent_swap'){const read=f.options.io.readSync;f.options.io.readSync=(fd,b,...args)=>{const n=read(fd,b,...args);if(f.opened.at(-1)===file)f.metadata.set('/etc/nginx/snippets',{ino:88});return n;};}if(mode==='second_metadata'){proof.preflight();f.metadata.set(file,{mode:0o100644,ino:88});assert.throws(()=>proof.perimeter());f.metadata.set(file,{mode:0o100644});assert.throws(()=>proof.perimeter());}else assert.throws(()=>proof.preflight());assert.equal(notes.at(-1).stage,'preflight_nginx');assert.ok(['include_metadata','include_snapshot_drift'].includes(notes.at(-1).reason));assert.equal(f.fds.size,0);assert.ok(f.buffers.every(b=>b.every(v=>v===0)));}});
...
test('main lexical alias refusal precedes snippet reads and every mutation boundary',async()=>{const m=await api();for(const literal of ['/dashboard/\\t18','/api/dashboard/\\r18','/dashboard/\\n18']){const f=withIncludes(fixture()),seen=[];f.options.notePhase=(stage,reason)=>seen.push({stage,reason});f.files.set(NGINX,NGINX_TEXT.replace('location / { proxy_pass http://127.0.0.1:3001; }','location "'+literal+'" { '+INCLUDES.map(p=>'include '+p+';').join('')+' }'));assert.throws(()=>m.createAbbottDeploymentProof(f.options).preflight(),{message:'ABBOTT_DEPLOY_PREFLIGHT_REFUSED'});assert.deepEqual(f.calls,[]);assert.ok(!f.opened.some(p=>INCLUDES.includes(p)));assert.deepEqual(seen.at(-1),{stage:'preflight_nginx',reason:'unsupported_location'});assert.equal(f.fds.size,0);assert.ok(f.buffers.every(b=>b.every(v=>v===0)));}});
