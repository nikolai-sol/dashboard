import fs from'node:fs';import path from'node:path';import{DEPLOY_STAGES}from'./abbott-deploy-transport.mjs';
export const DEPLOY_EVIDENCE_DIRECTORY='/Users/nafanya/ReportingDash/dashboard-next/.worktrees/abbott-runtime-isolation/.superpowers/sdd/.abbott-deploy-evidence';
const fail=()=>{throw Error('ABBOTT_DEPLOY_EVIDENCE_REFUSED');};
const valid=row=>row&&Object.keys(row).sort().join(',')==='exit,exitVerified,pid,stage,start'&&(row.pid===null||Number.isSafeInteger(row.pid)&&row.pid>0)&&(row.start===null||typeof row.start==='string'&&/^(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun) (?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) [ 0-3][0-9] [0-2][0-9]:[0-5][0-9]:[0-5][0-9] [0-9]{4}$/.test(row.start))&&typeof row.exit==='boolean'&&typeof row.exitVerified==='boolean'&&(!row.exitVerified||row.exit)&&DEPLOY_STAGES.includes(row.stage);
// The only production path is fixed. The filesystem seam is for temp fixtures.
export function createDeployEvidence(filesystem=fs){
 const root=DEPLOY_EVIDENCE_DIRECTORY,file=root+'/identity.json',next=file+'.next',uid=process.getuid();let directory,last;
