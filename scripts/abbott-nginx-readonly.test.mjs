...
test('known opaque block names are diagnosed but never traversed or accepted by deploy',async()=>{const m=await api();for(const body of ['if ($secret) { secret_nested value; }','types { private_nested secret; }','location / { limit_except GET { private_nested secret; } }']){const name=body.startsWith('location')?'limit_except':body.split(' ')[0];assert.deepEqual(m.diagnoseAbbottNginxText(base(body)),[name]);assert.throws(()=>m.validateAbbottNginxText(base(body)));assert.deepEqual(m.diagnoseAbbottNginxText(base(body+' location ~ /x { root /secret; }')),{reason:'parser_ambiguity'});}assert.deepEqual(m.diagnoseAbbottNginxText(base('private_block { sendfile on; }')),{reason:'parser_ambiguity'});});
...
test('batch diagnosis fails closed with no partial result for ambiguous TLS or unproven nesting',async()=>{
 const m=await api();assert.equal(typeof m.diagnoseAbbottNginxText,'function');
 for(const text of [base('sendfile on; location ~* /private { root /secret; }'),base('sendfile on; map $private $secret { root /secret; }'),base('sendfile on; location / { server { listen 443 ssl; } }'),base('sendfile on;')+'server { listen 443; server_name dashboards.adreports.ru; }',base('sendfile on;')+'}',base('sendfile on;').replace('443 ssl','80')])assert.deepEqual(m.diagnoseAbbottNginxText(text),{reason:'parser_ambiguity'});
});
