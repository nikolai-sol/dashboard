...
async function loader(mode,command,request){const m=await api();assert.equal(typeof m.ABBOTT_DEPLOY_LOADER,'string');const child=spawn(command?'/bin/sh':process.execPath,command?['-c',command.replace('/usr/bin/node',process.execPath)]:['--input-type=module','-e',m.ABBOTT_DEPLOY_LOADER],{env:{},stdio:['pipe','pipe','pipe','pipe']});const s=Buffer.from(mode==='success'?'export async function run(){return {status:"COMMITTED",record:null,diagnostic:{stage:"complete",reason:"none"}}}':mode==='import_error'?'private invalid syntax':mode==='crash'?'export async function run(){throw Error("private token")}':`import fs from'node:fs';export async function run(signal){fs.writeSync(3,'started');await new Promise(r=>signal.aborted?r():signal.addEventListener('abort',r,{once:true}));await new Promise(r=>setTimeout(r,10));return {status:${JSON.stringify(mode==='failure'?'REVIEW_REQUIRED':'RESTORED')},record:null,diagnostic:{stage:'compensation',reason:${JSON.stringify(mode==='failure'?'review_required':'restored')}}}}`),p=request===undefined?payload():Buffer.from(JSON.stringify(request)),out=[],err=[];let ready=false,expired=false;
...
test('cold acknowledged loader accepts only the fixed cold action',async()=>{
 const expectedCurrent={id:'a'.repeat(32),sourceSha:'b'.repeat(40),manifestDigest:'c'.repeat(64)};
 assert.equal((await loader('success',undefined,{action:'cold-restore-current',expectedCurrent})).status,'COMMITTED');
 assert.equal((await loader('success',undefined,{action:'cold-restore-other',expectedCurrent})).status,'REFUSED');
});
