...
test('actual Abbott CLI refusal is one closed line; exact capsule parses before dispatch',async()=>{
 const result=spawnSync(process.execPath,['scripts/deploy-runtime.mjs','deploy/abbott/release.json','deploy'],{cwd:new URL('..',import.meta.url),env:{APP_NAME:'private'},encoding:'utf8',timeout:3000});assert.equal(result.status,1);assert.match(result.stderr,/^ABBOTT_DEPLOY_REFUSED stage=unknown reason=failed\n$/);assert.equal(result.stdout,'');
...
test('fixed include diagnostics survive only strict nginx refusal framing',async()=>{const m=await api(),digest='d'.repeat(64);for(const reason of ['include_metadata','include_utf8','include_syntax','include_route','include_snapshot_drift']){const diagnostic={stage:'preflight_nginx',reason},wire=m.encodeAbbottDeployResult('REFUSED',null,digest,diagnostic);assert.deepEqual(m.parseAbbottDeployResult(wire,digest)?.diagnostic,diagnostic);assert.equal(m.formatAbbottDeployResult({status:'REFUSED',diagnostic}),`ABBOTT_DEPLOY_REFUSED stage=preflight_nginx reason=${reason}\n`);assert.throws(()=>m.encodeAbbottDeployResult('COMMITTED',null,digest,diagnostic));assert.throws(()=>m.encodeAbbottDeployResult('REFUSED',null,digest,{...diagnostic,stage:'preflight_current'}));assert.equal(m.parseAbbottDeployResult(wire.replaceAll(reason,reason+'_secret'),digest),null);}});
