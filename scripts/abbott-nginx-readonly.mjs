...
import{classifyAbbottNginxFirstRejection,inventoryAbbottNginxIncludes}from'./abbott-runtime-release-remote.mjs';
...
function readNginx(analyze,{io=fs,hostname=os.hostname,getuid=()=>process.getuid()}={}){
 ...
 try{
  if(getuid()!==0||hostname()!=='ybjqbzojln')fail();reason='metadata';const parents=ancestry(),before=io.lstatSync(file);
