...
import type { ChannelPerformanceItem, PlanVsFactItem } from '../src/lib/types';
...
async function main() {
  ...
  console.log(JSON.stringify({
    ...
    plan_vs_fact: data.plan_vs_fact?.map((r: PlanVsFactItem)=>({channel:r.channel,instrument:r.instrument,buy_type:r.buy_type,budget_plan:r.budget_plan,budget_fact:r.budget_fact,impressions_fact:r.impressions_fact,clicks_fact:r.clicks_fact,views_fact:r.views_fact,conversions_fact:r.conversions_fact,campaign_count:r.campaign_count,platforms:r.platforms})),
    channel_performance: data.channel_performance?.map((r: ChannelPerformanceItem)=>({channel:r.channel,instrument:r.instrument,spend_fact:r.metrics?.spend?.fact,spend_plan:r.metrics?.spend?.plan,status:r.metrics?.spend?.status}))
  }, null, 2));
}
