...
test('SQL extraction refuses unknown members, recursion, async maps and mutable inputs', () => {
  const unsafe = [
    ...
    'declare function runtimeSql():string; const queries:string[]=[]; function mutate(box:string[][]){const [alias=box]=[];alias[0].push(runtimeSql());} mutate([queries]); export const sql=queries.join("");',
    'declare function runtimeSql():string; const queries:string[]=[]; function mutate(box:{value:string[]}){const {alias=box}={};alias.value.push(runtimeSql());} mutate({value:queries}); export const sql=queries.join("");',
    'declare function runtimeSql():string; declare const maybe:string[][]|undefined; const queries:string[]=[]; function mutate(box:string[][]){const [alias=box]=[maybe];alias[0].push(runtimeSql());} mutate([queries]); export const sql=queries.join("");',
    'declare function runtimeSql():string; declare const flag:boolean; const queries:string[]=[]; const ui:string[]=[]; function choose(value:boolean,a:string[][],b:string[][]){return value?a:b;} function mutate(box:string[][],other:string[][]){const alias=choose(flag,box,other);alias[0].push(runtimeSql());} mutate([queries],[ui]); export const sql=queries.join("");',
    'declare function runtimeSql():string; const queries:string[]=[]; function mutate(box:string[][]){const [alias]=box;alias.push(runtimeSql());} mutate([queries]); export const sql=queries.join("");',
    'declare function runtimeSql():string; const queries:string[]=[]; function mutate(box:{value:string[]}){const {value}=box;value.push(runtimeSql());} mutate({value:queries}); export const sql=queries.join("");',
    'declare function runtimeSql():string; const queries:string[]=[]; const ui:string[]=[]; const alias=[]!==[]?queries:ui; alias.push(runtimeSql()); export const sql=queries.join("");',
    'declare function runtimeSql():string; const queries:string[]=[]; const ui:string[]=[]; const alias={}!=={}?queries:ui; alias.push(runtimeSql()); export const sql=queries.join("");',
    'declare function runtimeSql():string; const queries:string[]=[]; const ui:string[]=[]; const alias=[]+1==="1"?queries:ui; alias.push(runtimeSql()); export const sql=queries.join("");',
    ...
    'declare function runtimeSql():string; const queries:string[]=[]; const boxes=[[queries]]; boxes.map(box=>{box[0].push(runtimeSql());return 0;}); export const sql=queries.join("");',
    'declare function runtimeSql():string; const queries:string[]=[]; const boxes=[[queries]]; function touch(box:string[][]){box[0].push(runtimeSql());return 0;} boxes.map(touch); export const sql=queries.join("");',
    'declare function runtimeSql():string; const queries:string[]=[]; const boxes=[[queries]]; boxes.map(box=>{function touch(value:string[]){value.push(runtimeSql());} touch(box[0]);return 0;}); export const sql=queries.join("");',
    'declare function runtimeSql():string; const queries:string[]=[]; function mutate(boxes:string[][][]){boxes.map(box=>{box[0].push(runtimeSql());return 0;});} mutate([[queries]]); export const sql=queries.join("");',
    'declare function runtimeSql():string; const queries:string[]=[]; function pass<T>(value:T){return value;} function mutate(boxes:string[][][]){pass(boxes).map(box=>{box[0].push(runtimeSql());return 0;});} mutate([[queries]]); export const sql=queries.join("");',
    ...
    'declare function runtimeSql():string; const queries:string[]=[]; const ui:string[]=[]; type Box={"a.b":string[],a:{b:string[]}}; function live(flag:boolean,box:Box){box["a.b"].push(runtimeSql());if(flag)live(false,{"a.b":box.a.b,a:{b:box["a.b"]}});} live(true,{"a.b":ui,a:{b:queries}}); export const sql=queries.join("");',
    'declare function runtimeSql():string; const queries:string[]=[]; const ui:string[]=[]; function live(flag:boolean,current=queries){current.push(runtimeSql());if(flag)live(false);} live(true,ui); export const sql=queries.join("");',
    'declare function runtimeSql():string; const queries:string[]=[]; const ui:string[]=[]; function live(flag:boolean,current=flag?ui:queries){current.push(runtimeSql());if(flag)live(false);} live(true,ui); export const sql=queries.join("");',
  ...
  assert.deepEqual(extractStaticSql(`
    const safe=["SELECT * FROM dashboards"];
    const ui:string[]=[];
    function choose(flag:boolean,a:string[],b:string[]){
      return flag?a:b;
    }
    function live(flag:boolean,a:string[],b:string[],current=choose(flag,a,b)){
      current.push("label");
      if(flag)live(false,a,b);
    }
    live(true,safe,ui,ui);
  ...
  assert.deepEqual(extractStaticSql(`
    ...
    const safe=["SELECT * FROM dashboards"];
    const ui:string[]=[];
    function live(flag:boolean,fallback:string[],current=fallback){
      current.push(runtimeSql());
      if(flag)live(false,ui);
    }
    live(false,safe,ui);
    live(true,ui,ui);
    export const sql=safe.join("");
  ...
  assert.deepEqual(extractStaticSql(`
    ...
    const safe=["SELECT * FROM dashboards"];
    const ui:string[]=[];
    function live([flag,backup,current=backup]:[boolean,string[],string[]?]){
      current.push(runtimeSql());
      if(flag)live([false,backup,current]);
    }
    live([true,safe,ui]);
    export const sql=safe.join("");
  `, 'fixture.ts').statements, ['SELECT * FROM dashboards']);
  ...
  assert.deepEqual(extractStaticSql(`
    ...
    const safe=["SELECT * FROM dashboards"];
    const ui:string[]=[];
    function live(flag:boolean,current:string[],unused:string[]){
      current.push(runtimeSql());
      if(flag)live(false,current,unused);
    }
    live(true,ui,safe);
    export const sql=safe.join("");
  `, 'fixture.ts').statements, ['SELECT * FROM dashboards']);
  ...
  for (const receiver of [
    ...
    assert.deepEqual(extractStaticSql(`
      ...
      const safe=["SELECT * FROM dashboards"];
      const ui=["x"];
      function choose(flag:boolean,a:string[][][],b:string[][][]){return flag?a:b;}
      function mutate(flag:boolean,a:string[][][],b:string[][][]){
        (${receiver}).map(box=>{box[0].push(runtimeSql());return 0;});
      }
      mutate(false,[[safe]],[[ui]]);
      export const sql=safe.join("");
    `, 'fixture.ts').statements, ['SELECT * FROM dashboards']);
  }
  ...
  assert.deepEqual(extractStaticSql(`
    declare function runtimeSql():string;
    const safe=["SELECT * FROM dashboards"];
    function mutate(_:number){safe.push(runtimeSql());}
  ...
  assert.deepEqual(extractStaticSql(`
    const safe=["SELECT * FROM dashboards"];
    const ui=["label"];
    function outer(box:string[],other:string[]){
      function pass(value=box){return value;}
      return pass(other);
    }
    const alias=outer(safe,ui);
    alias.push("label");
    export const sql=safe.join("");
  `, 'fixture.ts').statements, ['SELECT * FROM dashboards']);
  ...
  assert.deepEqual(extractStaticSql(`
    const safe=["SELECT * FROM dashboards"];
    const ui:string[]=[];
    function mutate(box:string[][]){
      const [,alias=box[0]]=box;
      alias.push("label");
    }
    mutate([safe,ui]);
    export const sql=safe.join("");
  ...
  assert.deepEqual(extractStaticSql(`
    ...
    function pass<T>(value:T){return value;}
    function mutate(box:string[][],other:string[][]){
      const [alias=pass(box)]=[pass(other)];
      alias[0].push("label");
    }
    mutate([safe],[ui]);
    export const sql=safe.join("");
  ...
  assert.deepEqual(extractStaticSql(`
    ...
    function choose(flag:boolean,a:string[][],b:string[][]){return flag&&a||b;}
    function mutate(box:string[][],other:string[][]){
      const alias=choose(true,box,other);
      alias[0].push("label");
    }
    mutate([ui],[safe]);
    export const sql=safe.join("");
  ...
  assert.deepEqual(extractStaticSql(`
    ...
    function outer(flag:boolean){
      function inner(local:boolean){
        if(local&&flag)return safe;
        return ui;
      }
      return inner(true);
    }
    const alias=outer(false);
    alias.push("label");
    export const sql=safe.join("");
  `, 'fixture.ts').statements, ['SELECT * FROM dashboards']);
  ...
  assert.deepEqual(extractStaticSql(`
    ...
    function choose(flag:boolean,a:string[][],b:string[][]){return flag?a:b;}
    function mutate(box:string[][],other:string[][]){
      const alias=choose(false,box,other);
      alias[0].push("label");
    }
    mutate([safe],[ui]);
    export const sql=safe.join("");
