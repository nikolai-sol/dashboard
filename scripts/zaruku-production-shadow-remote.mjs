...
export function shadowTransportArguments(action,request,digest) {
  ...
  const code=`const CONTROL_FILES=${JSON.stringify(CONTROL_FILES)};const inspect=(${receiveControlPayload.toString()});let size=0;const chunks=[];try{for await(const bytes of process.stdin){size+=bytes.length;if(size>2097152)throw new Error();chunks.push(bytes);}const control=await inspect(Buffer.concat(chunks),${JSON.stringify(digest)},undefined,true);const module=await import('file://'+control.destination.path+'/scripts/zaruku-shadow-dispatch.mjs');const result=await module.dispatchStaged('worker',{action:${JSON.stringify(action)},request:${JSON.stringify(request)}});process.stdout.write(JSON.stringify(result)+'\\n');}catch{process.stderr.write('Zaruku shadow worker check failed\\n');process.exitCode=1;}`;
