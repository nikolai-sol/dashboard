...
import { assertRuntimeArtifact, verifyRuntimeArtifactBoot } from './abbott-runtime-artifact-policy.mjs';
import { createRuntimeInstaller } from './abbott-runtime-release-remote.mjs';
...
export function validateAuthority(filename) {
  ...
  const expected = RUNTIME_MANIFESTS.abbott;
  if (absolute !== path.join(ROOT, 'deploy/abbott/release.json') || !isDeepStrictEqual(JSON.parse(regular(absolute)), expected)) fail('Invalid fixed runtime authority');
  return expected;
}
...
export function preparePayload(authority, sourceSha) {
  ...
  for (const name of ['scripts/abbott-runtime-artifact-policy.mjs', 'package-lock.json', `deploy/${scope}/ecosystem.config.cjs`, `deploy/${scope}/start.cjs`]) authorityFile(name);
...
function prepareTransport(authority) {
  ...
  const worker = regular(path.join(ROOT, 'scripts/abbott-runtime-release-remote.mjs')).toString();
