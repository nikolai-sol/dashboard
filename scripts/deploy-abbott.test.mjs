...
test('acknowledged worker keeps finite include reasons without accepting forged private marker fields',async()=>{for(const reason of ['include_existing_abbott_route','include_existing_3004','include_variable_routing','include_regex_location','include_unsupported_location','include_unsupported_proxy_pass','include_unsupported_return','include_unsupported_add_header','include_unsupported_other','include_authority','include_private_token']){const f=fixture();try{f.platform.deploymentPreflight=note=>{note('preflight_nginx',reason);throw Object.assign(Error('private-token'),{stage:'complete',reason:'include_private_token',stdout:'private-token'});};const result=await f.installer.transactAcknowledged({action:'inspect'},new AbortController().signal,f.platform);assert.deepEqual(JSON.parse(JSON.stringify(result)),{status:'REFUSED',record:null,diagnostic:{stage:'preflight_nginx',reason:reason==='include_private_token'?'unknown':reason}});assert.equal(f.events.length,0);assert.equal(fs.existsSync(f.map('/var/www/.dashboard-abbott-deploy.lock')),false);assert.doesNotMatch(JSON.stringify(result),/private/);}finally{f.cleanup();}}});
