...
export async function loadDashboardReadModel(input: Readonly<{
  ...
  for (const source of input.registration.profile.sources) {
    ...
    try {
      ...
      datasets[source.sourceKey] = await input.execute({ name: "dataset", scope, period, publicationId: input.publicationId, filters: input.filters }) as DatasetMeta;
  ...
  return { gsc, indexing, datasets, trafficComparison };
}
