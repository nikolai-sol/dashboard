...
export default async function SiteSeoDashboardPage({ params, searchParams }: Readonly<{ params: Promise<{ siteSlug: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }>) {
  ...
  const loaded = await (async () => {
    try {
      ...
      const publicationMatches = intentPublicationMatches(model.targetIntent, expectedIntentPublication, intentReviewRequested);
      return { values, request, model, availableWeeks, publicationMatches };
    } catch {
      return null;
    }
  })();
  ...
  const { values, request, model, availableWeeks, publicationMatches } = loaded;
