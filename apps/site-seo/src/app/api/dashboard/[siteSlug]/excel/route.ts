import { createExcelExportHandler } from "../../../../../lib/route-handlers.ts";
...
export async function GET(request: Request, context: { params: Promise<{ siteSlug: string }> }): Promise<Response> {
  try {
    ...
    try { return createExcelExportHandler({ registration: runtime.registration, credentialVersion: session.credentialVersion, getSession: async () => session, execute: runtime.execute })(parseDashboardReadRequest(new URL(request.url), siteSlug, runtime.registration.profile.businessTimezone)); }
    catch { return Response.json({ error: "invalid_request" }, { status: 400, headers: { "cache-control": "private, no-store" } }); }
  } catch { return Response.json({ error: "site_runtime_unavailable" }, { status: 503, headers: { "cache-control": "private, no-store" } }); }
}
