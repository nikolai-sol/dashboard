...
export async function GET(request: Request, context: { params: Promise<{ siteSlug: string }> }): Promise<Response> {
  try {
    ...
    try { return createPdfExportHandler({ registration: runtime.registration, credentialVersion: session.credentialVersion, getSession: async () => session, execute: runtime.execute })(parseDashboardReadRequest(new URL(request.url), siteSlug, runtime.registration.profile.businessTimezone)); }
