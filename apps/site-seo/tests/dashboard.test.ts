...
test("standard sheets use the accepted Zaruku-style stack of focused panels", () => {
  ...
  const seoOs = { ...seoMeta, kind: "seo_os" as const, observationPeriod: selection.traffic.primary, observationDate: "2026-09-11", selectionPeriod: selection.traffic.primary, positions: [], rows: [], recommendations: [{ kind: "content", topic: "Онкология", pageUrl: "/page", action: "Обновить", sourceIds: ["webmaster"], sourcePeriods: ["2026-W37"], ruleVersion: "v1", publicationStatus: "published" }], tasks: [{ id: "task-1", status: "open" }] };
