...
13. Zaruku Geo tab now avoids duplicate country/city bar lists. It renders one product panel, `Карта спроса по России`, with an inline Russia SVG outline and city bubbles. It is backed by `zaruku_seo.map_city_demand` from Metrika `regionCity × /map`; bubble size is visits and each city shows visits plus share of total map demand. Important product wording: this is only visits to the `/map/` organization-map section, not all geo traffic for the site.
