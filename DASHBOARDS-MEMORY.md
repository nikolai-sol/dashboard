...
11. Zaruku portal BI dashboard was added as `dashboard_type = zaruku_bi`:
    ...
    - Zaruku Geo tab renders one focused `Карта спроса по России` panel instead of separate country/city bar lists. It uses an inline Russia SVG outline plus city bubbles. Data comes from `zaruku_seo.map_city_demand` / Metrika `regionCity × /map`; bubble size is visits and the city legend shows visits plus share of total map demand. This is only visits to `/map/`, not all geo traffic for the site.
