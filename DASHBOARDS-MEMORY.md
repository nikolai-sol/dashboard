...
11. Zaruku portal BI dashboard was added as `dashboard_type = zaruku_bi`:
    ...
    - branch target for the onsite Yandex Metrika source: all dashboard reads use canonical MySQL traffic/page facts plus `canonical_fact_metrika_breakdowns_daily` for search engines, phrases, organic landings, devices, geo, browser/OS, and inferred age/gender/interests; `canonical_metrika_breakdown_coverage_daily` records successful/empty breakdown coverage. This is not production state until the migration, deploy, and backfill are accepted.
