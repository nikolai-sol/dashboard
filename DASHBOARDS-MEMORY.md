...
11. Zaruku BI dashboard uses the Abbott BI data shape with `dashboard_type = zaruku_bi`; its traffic summary prefers canonical Yandex Metrika `analytics_scope = 'other'` traffic-source rows when present, and falls back to `analytics_scope = 'traffic'` UTM rows otherwise. Page stats use canonical `analytics_scope = 'page'`. User ID analytics are Abbott/Bitrix-specific and hidden for Zaruku reports.
