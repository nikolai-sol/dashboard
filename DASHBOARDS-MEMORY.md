...
18. Zaruku uses one dashboard UI contract as of 2026-07-27:
    ...
    - Audience segment facts can exist while every panel appears unavailable if MySQL prepared execution binds `LIMIT ?`; `buildMetrikaSegmentQueries` must interpolate only a clamped integer limit (`1..10000`) and keep all data values parameterized
