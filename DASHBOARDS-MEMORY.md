...
11. Zaruku portal BI dashboard was added as `dashboard_type = zaruku_bi`:
    ...
    - Zaruku Geo tab renders one focused `Карта спроса по России` panel instead of separate country/city bar lists. It uses `zaruku_seo.map_city_demand` from Metrika `regionCity × /map`; bubble size is visits and the city legend shows visits plus share of total map demand.
