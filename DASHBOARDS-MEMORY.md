...
13. Zaruku Geo tab now avoids duplicate country/city bar lists. It renders one product panel, `Карта спроса по России`, backed by `zaruku_seo.map_city_demand` from Metrika `regionCity × /map`; bubble size is visits and each city shows visits plus share of total map demand. The next implementation plan for returning content and GSC enrichment is `dashboard-next/docs/superpowers/plans/2026-07-19-zaruku-returning-content-gsc-enrichment.md`.
...
15. Zaruku GSC enrichment now includes `country_summary` plus device summary panels derived from existing `canonical_fact_gsc_queries_daily`. These are dashboard-side SQL aggregations, not new Google API calls. Optional future GSC expansion remains `searchAppearance` and result `type` canonical collection after product approval.
