...
15. Zaruku returning content is canonical as of 2026-07-19. Legacy `yandex_metrika_returned` is no longer the product read model for Zaruku. Root collector `fetch_yandex_metrika_returning_canonical.py` writes `canonical_fact_metrika_returning_pages_daily` in `/root/reportingdash-canonical`. The intended cron is daily at `06:18`, but on 2026-07-27 the installed command was confirmed broken because it passes the removed `--counter-id 66624469` option; the log repeats `unrecognized arguments` and no cron collection has succeeded after 2026-07-21. A successful backfill on 2026-07-24 populated facts through 2026-07-23. The Behavior tab panel `Возвратный контент` reads the canonical table and shows visits plus 1-day / 2–7-day / 8–31-day returning-user buckets. Quality/source freshness includes `yandex_metrika_returning`.
