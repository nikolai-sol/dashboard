...
19. Zaruku confirmed query landings were validated further on 2026-07-28. The current W30 read has 4,528 unified keys, 3,259 with a same-row GSC page, and 1,269 Webmaster-only keys. Only 695 of 3,958 Webmaster impressions (17.5594%, partial W30) belong to queries that pass the current GSC-only filter, so the UI now states explicitly that confirmation currently comes only from Google and that Yandex may use another page. A strict URL count for every Webmaster-only query is not derivable from separate query/page facts; the `popular_complementary_indicator` proxy covers 491 normalized URLs but is representative-only. Enhanced Export task `26224340-8a5c-11f1-b8b7-21c9fbaae2a2` then proved the exact Yandex contract with 152 same-row `query + path + clicks` CSV rows for one URL on 2026-07-22 (13 clicks, 191 impressions). One free quota unit was used; no collector, table, migration, cron, or deployment change followed.
