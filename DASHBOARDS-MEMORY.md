...
11. Zaruku portal BI dashboard was added as `dashboard_type = zaruku_bi`:
    ...
    - the connected onsite Yandex Metrika source reads canonical MySQL traffic/page facts plus `canonical_fact_metrika_breakdowns_daily` for search engines, phrases, organic landings, devices, geo, browser/OS, and inferred age/gender/interests; `canonical_metrika_breakdown_coverage_daily` records successful/empty breakdown coverage, and the dashboard does not call the live Metrika API
    ...
    - Detailed GSC panels remain available under expandable diagnostics for devices, brand vs non-brand, Search appearances, and result types. Brand bucket currently matches `zaruku`, `заруку`, `за руку`, and `зараку`; everything else is `non_brand`.
    - AI summary and AI chat are future extension points at the SEO executive/read-model boundary. No placeholder UI, endpoint, database table, or collector has been added yet.
    ...
    - Zaruku Quality tab source freshness is intentionally technical (`cron`, `collector`, `rows`). Its displayed last successful update uses the latest successful collection of any run type, so a reviewed backfill is not hidden behind an older cron success. It hides stale historical cron errors after a newer success and shows errors only when the latest failed/partial cron is newer than or equal to the latest success. Product freshness is calendar-based: fact age `0..3` days is current and `>3` days is delayed; collector `expected_frequency_hours` remains separate schedule metadata. A newer failed cron/API run remains `failed` even while the facts are current.
...
15. Zaruku returning content is canonical as of 2026-07-19. Legacy `yandex_metrika_returned` is no longer the product read model for Zaruku. Root collector `fetch_yandex_metrika_returning_canonical.py` writes `canonical_fact_metrika_returning_pages_daily` in `/root/reportingdash-canonical`. The daily `06:18` cron was repaired on 2026-07-28: the invalid `--counter-id` selector was replaced by `--account-id 66624469`, and `--run-type cron` was restored so scheduled runs remain visible to freshness logic and retain the cron duplicate guard. The exact cron path produced run `1694` (`cron/success`) and wrote 484 rows for `2026-07-24..2026-07-27`; the live dashboard then reported `yandex_metrika_returning` as `healthy`. The pre-change crontab is recoverable from `/root/crontab.backup-rd08-20260728T080419Z`. The Behavior tab panel `Возвратный контент` reads the canonical table and shows visits plus 1-day / 2–7-day / 8–31-day returning-user buckets. Quality/source freshness includes `yandex_metrika_returning`.
...
18. Zaruku uses one dashboard UI contract as of 2026-07-27:
    ...
    - Zaruku explanatory info controls use one shared portal-rendered popover: Overview KPI and period explanations are not clipped by panels, do not reflow their rows, support hover, focus, and tap, and remain viewport-contained. The north-star heading has no duplicate general info icon. Shared `.zaruku-table` cells retain a 12 px horizontal inset so table content does not touch frame or row dividers.
    ...
    - Audience segment facts can exist while every panel appears unavailable if MySQL prepared execution binds `LIMIT ?`: production MySQL 8.0.42 returns `ER_WRONG_ARGUMENTS` / errno `1210` because mysql2 sends a JavaScript number through the binary protocol as `DOUBLE`. Zaruku Metrika query builders must interpolate only trusted/clamped integer presentation limits and keep account IDs, report keys, and dates parameterized. `buildZarukuMetrikaBreakdownQueries` is guarded by a regression test that rejects every `LIMIT ?` across all twelve reports.
...
19. Zaruku confirmed query landings were validated further on 2026-07-28. The pre-pair W30 read had 4,528 unified keys, 3,259 with a same-row GSC page, and 1,269 Webmaster-only keys; the historical Google-only confirmed cohort carried 695 of 3,958 Webmaster impressions (17.5594%, partial W30). A strict URL count for every Webmaster-only query was not derivable from separate query/page facts; the `popular_complementary_indicator` proxy covered 491 normalized URLs but remained representative-only. Enhanced Export task `26224340-8a5c-11f1-b8b7-21c9fbaae2a2` proved the exact Yandex contract with 152 same-row `query + path + clicks` CSV rows for one URL on 2026-07-22 (13 clicks, 191 impressions). On 2026-07-29 the standard exact-URL probe matched that artifact with zero differences, migration `045` and manual run `1715` established the bounded canonical pair layer, and dashboard `main` began accepting GSC or exact Webmaster pairs. Production application deployment and the weekly cron remain gated as recorded above.
20. Zaruku organic landing rows are page-grain in the dashboard read model. Canonical Metrika `organic_landing` facts arrive as `searchEngine × startURL`; the dashboard must first aggregate them by exact normalized `page_url`, then use canonical `analytics_scope = 'page'` `page_title` as the row label, falling back to the URL. A search-engine dimension such as `Google, search results` or `Yandex Mobile` must never become the page label.
21. Zaruku's `search_engines` panel is engine-family grain. Canonical Metrika `ym:s:searchEngine` values whose id or label starts with `google` are published as one `Google` row; values starting with `yandex` or `яндекс` are published as one `Яндекс` row. Mobile app, images, mobile, and search-result subtypes are not separate bars in this panel. Volumes are summed and rate metrics are visit-weighted; other engines remain excluded from this two-family comparison.
22. Zaruku GSC landing-page aggregation ranks and limits rows inside each `week_key`, then returns the available weeks newest-first; a single global `LIMIT 200` must never allow an older week to consume the whole read model. On 2026-08-03 the active production bundle had regressed to the global ascending limit and returned only W28 even though canonical MySQL contained W31 landing facts. App release `20260803205516-4edb88f` restored the per-week query. The production API then returned 200 landing rows for each of W28-W31, with W31 first; `/kompleksnoe_genomnoe_profilirovanie/` returned 12 Google impressions, 0 clicks, 0% CTR, and average position 2.25 for W31. The same release contains long page labels inside the first table column with `overflow-wrap: anywhere` and hidden overflow. Browser console, local/public health, and loopback-listener checks passed. No collector, canonical data, migration, cron, secret, Abbott dashboard, or other dashboard logic changed.
23. Zaruku Semantic Health follows the same explicit source-week fallback contract as the unified SEO tables. Production SOV currently contains only W29 (12 clusters, source period `2026-06-13..2026-07-10`), so selecting W31 must chart and tabulate W29 while displaying `W31 недоступна, показано W29`; it must never label the W29 facts as W31 or show the source as disconnected. App release `20260803210522-c553615` deployed this correction. Production browser verification showed the W29 axis, the explicit fallback note, all 12 rows, zero console errors/warnings, healthy local/public endpoints, and an online loopback-only PM2 process. No source facts, collector, migration, cron, secret, Abbott dashboard, or other dashboard behavior changed.
