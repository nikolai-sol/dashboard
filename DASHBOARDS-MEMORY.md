...
22. Zaruku GSC landing-page aggregation must sort `week_key DESC` before applying `LIMIT 200`. The SEO UI intentionally selects the GSC read model's latest week independently from the SEO OS selector, so ascending week order can fill the bounded page result with an older week and leave every Google landing-page column empty. App release `20260729155705-1b2cd27` deployed the newest-first correction on 2026-07-29. A production-backed W30 read returned 200 selected landing rows, and all 200 produced non-null Google metric groups in the unified page model. No period logic for Webmaster, Metrika, or SEO OS changed.
