...
- URL/page facts come from Yandex Webmaster `query-analytics/list` with `text_indicator = URL`; default `YANDEX_WEBMASTER_SEARCH_LOCATION = ALL_LOCATIONS`, matching the Webmaster UI screenshot. The restored page writer and Webmaster floor/span `2/3` were deployed atomically on 2026-07-28 under tag `20260728T171112Z`, but no end-to-end collector run has occurred; historical page rows do not prove the restored writer is operational.
...
- SELECT-only gap audit for `2026-07-01..21`: Zaruku Metrika has source rows on every date; GSC core has rows on every date; Webmaster query/summary/page are all absent for `2026-07-01..09`, while only Webmaster page facts are absent for `2026-07-10..12`. The separate failed-lineage scope remains `2026-07-14..17` with 1,698 rows. Wait for the scheduled 2026-07-29 Webmaster cron at 06:50, require `status=success`, `error_count=0`, and advancing query/page maxima, then refresh the SELECT-only snapshot before approving any exact backfill list. The GSC 06:55 gate and `--layers optional` remain separate.
...
16. The Webmaster page writer, returning/GSC/Webmaster floor windows, and shared RD-11 health/Telegram model are committed. On 2026-07-28, five independent runtime files were deployed atomically with backups under tag `20260728T171112Z`; merged-main verification passed 101 focused tests and `py_compile`, and post-deploy SHA/mode/import plus SELECT-only health checks passed. Generic Metrika floor/span remains undeployed because its active collector belongs to the attested private runtime and requires a committed manifest/revision rollout. No collector/backfill run, cron/schema/secret change, or Telegram send occurred.
