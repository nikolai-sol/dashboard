...
a mode-`0600` temporary file, never appears in argv or evidence, and is deleted on exit.
...
- compares PDF and Excel status, semantic headers, and bodies byte-for-byte;
...
material. The output files are `summary.json`, `endpoint-parity.json`,
...
positive case covers health, unauthorized metadata, manager JSON, PDF, and Excel. Thirteen negative
cases prove nonzero exit for a historical total change, missing direct owner addition, missing Alice
month, Wordstat coverage change, canonical coverage change, source-health change, auth metadata
change, PDF difference, Excel difference, semantic health-header difference, another-runtime SHA
mutation, a same-value SHA-file rewrite detected by file identity, and an Abbott route marker in a
compiled Zaruku artifact file. These are
behavioral fixtures, not source-regex assertions.

## Fresh verification evidence

The final uninterrupted `npm run predeploy:verify` exited zero after a successful `npm ci`. Its order
...
- Runtime artifact policy: 144/144. Shadow verifier: 1 positive + 13 negative fixtures.
...
Task 7 changes only the shadow verifier and its tests, predeploy gate and contract test, `OPS.md`, and
this report. There were no source API/OAuth calls, database reads or writes, migration actions,
