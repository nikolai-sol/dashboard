...
- Zaruku source freshness uses one product rule for Metrika, returning content, Webmaster, and GSC: compare the latest fact date with the UTC calendar date at the start of the current day; age `0..4` days is current and age `>4` days is delayed. `expected_frequency_hours` remains collector schedule metadata and must not drive this product status. Yandex documents Webmaster query statistics as updated daily, but does not publish a fixed SLA; the 4-day allowance is ReportingDash policy based on the observed lag of the slowest source. A newer failed cron/API run remains `failed` even when the latest facts are still inside the four-day window.
