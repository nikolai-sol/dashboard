...
- Abbott service routes belong in the registry, not in per-batch exclusions. A `reject` URL-decision is bound to `exclusion.approval_batch_id = candidate_batch.id`, so it must be reissued in every release; a registry row persists and gives the page its real `Не относится / служебная` label. Twelve service routes were added to `registry1-svc.xlsx`/`registry2-svc.csv`; only the 58 URLs absent from the reviewed registry stayed as reviewed exclusions.
- Direction lives in two unrelated places and they are constantly confused: dashboard tab 2 reads the **user's** direction from `portal_user_directions_private`, tabs 3/3.1/returning read the **page's** direction from `portal_content_catalog`. Material classification never affects tab 2. That dictionary stops at `USER_ID 81311`, so it covers 9.4% of active July users; only a fresh Bitrix export fixes it.
- Hub pages carry their direction in the query filter, not in the catalog. App revision `e89f760` resolves `/preparation/?direction[]=` only when it names exactly one known section and the catalog itself has no decision; section ids are derived from the catalog's own labels so the mapping cannot drift from the taxonomy.

- **Release composition (learned 2026-08-21).** A content release wipes `portal_content_catalog_mnn`; the MNN successor preserves the content catalog. MNN must therefore be applied **last**: content batch → activate → `abbott_mnn_successor materialize` on top. Releases 39/40 were built outside the batch pipeline on 2026-08-19 and record `code_revision dea003e`, a revision absent from `content-runtime/releases/`; the MNN runtime lives at `/root/reportingdash-private/abbott/mnn-successor-runtime-dea003e`, uses its own Python 3.8 venv (run it through `agents/abbott_page_classifier/python311_runtime.sh`), and a release it produced can only be activated by **its own** `abbott_release_operator.py` — the content operator rejects it with "Frozen baseline source manifest is invalid". `abbott_mnn_successor` also demands that its predecessor be both the active release and one materialized from a batch (`assert_historical_batch_evidence`).
- **Never delete or insert registry rows in place.** Provenance binds to `source_row_ordinal`; removing even blank rows shifts every later ordinal and breaks the link to accepted events, surfacing as `SOURCE_PROVENANCE_COLLISION` at materialize. Append at the end of a sheet, or add a new sheet. The pipeline is idempotent on input hashes: identical registries re-use the same run and batch, so a new batch requires a changed hash.
- **The candidate gate demands `non_content_unresolved == 0`**, counted over fact rows whose normalized path has neither a catalog row nor a reviewed exclusion. Accepted `reject` URL-decisions create those exclusions — see `ops/abbott/README.md`. Their decision file must stay under 8 MB (`local_acceptance.py`), which forces terse reject wording at scale.
- **Root blocker as of 2026-08-21:** the portal mints a unique URL per event instance — 13 689 `/events/*` paths for 15 732 views, 13 609 of them with ≤1 view, including loading-state text turned into slugs. Rejecting them wholesale collides on provenance ordinals. The fix is to normalize `/events/<slug>` → `/events/` where facts are written; hand-editing `canonical_fact_metrika_site_analytics_daily` is not an option because its `scope_hash` does not reproduce from the collector formula.
- Clean MNN snapshot `116` (0 rejected values, same 1048 mappings as the dirty snapshot `102`) is imported and carried by release `42`. The 167 rejections in snapshot 102 were placeholder drug names (`-`, `нет бренда`, `не найдено`, `скрыто`); the parser drops them by design, and the content gate refuses any snapshot with a non-zero `rejected_row_count`.
